...
/**
 * HtmlSave — promote an HTML artifact into the vault as an entry. Mirrors the
 * MediaSave / ChatActions destination-folder picker, but stores the markup in
 * `content` with `mediaType: "text/html"` so the vault renders it sandboxed.
 */
export function HtmlSave({
  html,
  defaultTitle = "Generated artifact",
}: {
  html: string;
  defaultTitle?: string;
}) {
    ...
    <div className="relative inline-flex shrink-0 items-center">
      ...
      {open && (
        <>
          ...
          />
          <div className="absolute left-0 top-full z-50 mt-2 w-80 rounded-xl border border-border-strong bg-surface p-3 shadow-2xl shadow-black/60 [animation:cairn-rise-in_150ms_ease-out]">
